...
class Persist:
    ...
    def persist_data(self, df: DataFrame) -> None:
        ...
        # Write DataFrame into Cassandra keyspace.
        cassandra_df = df
        self.write_dataframe_into_cassandra(cassandra_df)
    ...
    def write_dataframe_into_csv(self, df: DataFrame) -> None:
        ...
        df.write.option("header", "true").mode("overwrite").csv("output/etl_course")

    def write_dataframe_into_mysql(self, df: DataFrame) -> None:
        print("persisting dataframe into mysql")
        df.write \
            .format("jdbc") \
            .option("url", "jdbc:mysql://localhost:3306/etl_course") \
    ...
    def write_dataframe_into_cassandra(self, df: DataFrame) -> None:
        print("persisting dataframe into cassandra")
        df.write \
            .format("org.apache.spark.sql.cassandra") \
            .mode("append") \
            .options(table="course2", keyspace="etl_course") \
            .save()
